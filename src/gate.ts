...
<title>${title}</title><link rel="icon" type="image/svg+xml" href="/icons/favicon.svg"><style>${STYLE}</style></head>
<body><main>${MARK}${body}</main></body></html>`;

export function signInPage(next: string): Response {
  const href = `/auth/login?next=${encodeURIComponent(next)}`;
  return new Response(
    page(
      "Paper Archive",
      `<h1>Paper Archive</h1><p class="ja">ペーパーアーカイブ</p>
       <p>Scan mail → know what it wants → know whether to keep the original.</p>
       <p>郵便物をスキャン → 内容と必要な対応を把握 → 原本を残すか判断</p>
       <a class="btn" href="${href}">Sign in with Google · Googleでログイン</a>
       <p class="who">Invite only · 招待制</p>`,
    ),
    { status: 401, headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store" } },
  );
}

export function notInvitedPage(email: string): Response {
  const safe = email.replace(/[<>&"]/g, "");
  return new Response(
    page(
      "Paper Archive — invite only",
      `<h1>Invite only</h1><p class="ja">招待制のアプリです</p>
       ...
       <p>このアカウントは登録されていません。管理者に追加を依頼してから、もう一度ログインしてください。</p>
       <a class="btn" href="/auth/logout">Try another account · 別のアカウントで</a>`,
    ),
